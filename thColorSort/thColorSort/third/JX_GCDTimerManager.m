...
static NSString *TimerState_Run = @"TimerState_Run";
static NSString *TimerState_Cancel = @"TimerState_Cancel";
static NSString *TimerState_Suspend = @"TimerState_Suspend";

@interface JX_GCDTimerManager()
@property (nonatomic, strong) NSMutableDictionary *timerContainer;
@property (nonatomic, strong) NSMutableDictionary *actionBlockCache;
@property (nonatomic, strong) NSMutableDictionary *timerStateContainer;//保存每个timer的状态
@end
                                ...
                                action:(dispatch_block_t)action
{
    ...
    if (!timer) {
        ...
        [self.timerStateContainer setObject:TimerState_Run forKey:timerName];
    }
...
- (void)cancelTimerWithName:(NSString *)timerName
{
    ...
    if ([[self.timerStateContainer valueForKey:timerName] isEqualToString:TimerState_Suspend]) {
        dispatch_resume(timer);
    }
    [self.timerContainer removeObjectForKey:timerName];
    [self.timerStateContainer removeObjectForKey:timerName];
...
- (void)pauseTimerWithName:(NSString *)timerName{
    ...
    if (timer) {
        if ([[self.timerStateContainer objectForKey:timerName] isEqualToString:TimerState_Run]) {
            [self.timerStateContainer setObject:TimerState_Suspend forKey:timerName];
...
-(void)resumeTimerWithName:(NSString *)timerName{
    ...
    if (timer) {
        if ([[self.timerStateContainer objectForKey:timerName] isEqualToString:TimerState_Suspend]) {
            [self.timerStateContainer setObject:TimerState_Run forKey:timerName];
            dispatch_resume(timer);
        }
    }
...
- (void)cancelAllTimer
{
    ...
    [self.timerStateContainer enumerateKeysAndObjectsUsingBlock:^(NSString *timerName, dispatch_source_t timer, BOOL *stop) {
        [self.timerStateContainer removeObjectForKey:timerName];
    }];
}
