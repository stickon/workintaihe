...
@interface IRDiffAdvanced31View ()<UITableViewDataSource,UITableViewDelegate,TableViewCellChangedDelegate,FoldSectionHeaderViewDelegate>
{
    BOOL dataLoaded;
    Byte waveType;
...
@end
@implementation IRDiffAdvanced31View
...
-(instancetype)init{
    if (self = [super init]) {
        UIView *subView = [[[NSBundle mainBundle]loadNibNamed:@"IRDiffAdvancedView" owner:self options:nil] firstObject];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section ==0) {
        if (indexPath.row ==0) {
            ...
            cell.textField.text = [NSString stringWithFormat:@"%d",(device->data1[indexPath.row-1])+(device->data2[indexPath.row-1])*256];
    ...
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {//设置杂质大小
            ...
            cell.irDiffSpotLabel.hidden = NO;
            cell.irDiffTitleLabel.hidden = NO;
            cell.irDiffTextField.hidden = NO;
            cell.sizeLabel.text = kLanguageForKey(228) ;//红外比值时，杂质大小控件名复用为最小点数
            cell.irDiffTitleLabel.text = kLanguageForKey(184);
            cell.irDiffTextField.text = [NSString stringWithFormat:@"%d",device->irDiffOrSpot[0]*256+device->irDiffOrSpot[1]];
            if (device->IsirDiffOrSpot) {
                cell.isIRDiff = 1;//比例 0是非红外
                cell.irDiffSpotLabel.text = kLanguageForKey(226);
            }else{
                cell.isIRDiff = 2;//点数
                cell.irDiffSpotLabel.text = kLanguageForKey(227);
            }
            ...
            cell.lightTitleLabel.hidden = YES;
            cell.lightRadioBtn.hidden = YES;
            ...
            NSArray *_irDiffArray = @[irCompareString1,irCompareString2];
            
            cell.colorTitleLabel.text = [_irDiffArray objectAtIndex:device->sense.color];
            ...
            if (tag == 1) {
                if (bsend) {
                    ...
                    for (int i =0; i<device->machineData.chuteNumber; i++) {
                        ...
                        device->data1[i] = value%256;
                        device->data2[i] = value/256;
                    }
                    [self reloadRows];
                }
