...
static NSString *irSharpenCellIdentifier = @"IRSharpenTableViewCell";

@interface IRColorAdvanced31View ()<UITableViewDataSource,UITableViewDelegate,TableViewCellChangedDelegate,FoldSectionHeaderViewDelegate>
{
    BOOL dataLoaded;
    Byte waveType;
...
@end
@implementation IRColorAdvanced31View
...
-(instancetype)init{
    if (self = [super init]) {
        UIView *subView = [[[NSBundle mainBundle]loadNibNamed:@"IRColorAdvanced31View" owner:self options:nil] firstObject];
...
-(void)initView{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:irSharpenCellIdentifier bundle:nil] forCellReuseIdentifier:irSharpenCellIdentifier];
    [self.useLabel setText:kLanguageForKey(35)];
...
-(void)refreshCurrentView{
    ...
    [gNetwork getIRSharpenWithAlgorithmType:self.type];
}
-(void)updateWithHeader:(NSData *)headerData{
    const unsigned char* header = headerData.bytes;
    if (header[0] == 0x04 && (header[1] == 0x13 || header[1] == 0x17)) {
        dataLoaded = true;
    ...
    if (header[0] == 0x04 && (header[1] == 0x12)) {
        [[NetworkFactory sharedNetWork]sendToGetSenseAdvancedData:self.type IsIR:1];
    }
    if (header[0] == 0x04 && header[1] == 0x18) {
        [gNetwork getIRSharpenWithAlgorithmType:self.type];
    }
    if (header[0] == 0x05) {
        if (_timer.isValid) {
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1) {
        ...
        if (indexPath.row == 1) {
            IRSharpenTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:irSharpenCellIdentifier forIndexPath:indexPath];
            cell.indexPath = indexPath;
            cell.delegate = self;
            cell.SharpenUseBtn.tag = 1;
            cell.SharpenEdit.tag = 2;
            [cell.SharpenUseBtn setTitle:@"use" forState:UIControlStateSelected];
            [cell.SharpenUseBtn setTitle:@"nouse" forState:UIControlStateNormal];
            cell.SharpenEdit.text = [NSString stringWithFormat:@"%d",device->irSharpenValue];
            if (device->irSharpenEnable == 1) {
                cell.SharpenUseBtn.selected = YES;
                cell.SharpenUseBtn.backgroundColor = [UIColor greenColor];
            }else{
                cell.SharpenUseBtn.selected = NO;
                cell.SharpenUseBtn.backgroundColor = [UIColor TaiheColor];
            }
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
            return cell;
        }
...
-(void)cellValueChangedWithSection:(long)section row:(long)row tag:(long)index value:(NSInteger)value
{
    ...
    if (section == 0)
    {
        ...
        if (row == 0) {
            ...
            [gNetwork setIRSharpenWithAlrotithmType:_type Type:index Value:value];
        }
