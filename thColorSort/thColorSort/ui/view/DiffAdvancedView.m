...
@interface DiffAdvancedView()<UITableViewDataSource,UITableViewDelegate,TableViewCellChangedDelegate,FoldSectionHeaderViewDelegate>
{
    BOOL dataLoaded;
    Byte waveType;
...
@end
@implementation DiffAdvancedView
...
-(void)cellValueChangedWithSection:(long)section row:(long)row tag:(long)index value:(NSInteger)value
{
    ...
    if (section == 0)
    {
        ...
        }else if (row == device->machineData.chuteNumber+1)//整体料槽调整
        {
            ...
            {
                if (index == 1) {
                    [[NetworkFactory sharedNetWork]sendAlgorithmSenseValueWithAjustType:1 Sorter:row data:value algorithmType:_type FirstSecond:currentSelectFrtsnd ValueType:currentSelectColordiffType IsIR:0];
                }
                if (index == 2) {
                    [[NetworkFactory sharedNetWork]sendAlgorithmSenseValueWithAjustType:2 Sorter:row data:value algorithmType:_type FirstSecond:currentSelectFrtsnd ValueType:currentSelectColordiffType IsIR:0];
                }
